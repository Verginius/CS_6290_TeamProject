...
contract SetupScenarios is Script, StdCheats {
    ...
    function _setupFlashLoanProvider() internal {
    ...
    function _writeEnvSimulation() internal {
        // Read FlashLoanProvider address deployed in Deploy.s.sol
        address flashLoanProvider = vm.envAddress("FLASH_LOAN_PROVIDER_ADDRESS");

        string memory envFile = string(
            abi.encodePacked(
                "# Auto-generated by script/SetupScenarios.s.sol\n",
                "GOV_TOKEN_ADDRESS=",
                vm.toString(deployedContracts.govToken),
                "\n",
                "GOVERNOR_VULNERABLE_ADDRESS=",
                vm.toString(deployedContracts.governorVulnerable),
                "\n",
                "GOVERNOR_DEFENSES_ADDRESS=",
                vm.toString(deployedContracts.governorDefended),
                "\n",
                "MOCK_TREASURY_ADDRESS=",
                vm.toString(deployedContracts.mockTreasury),
                "\n",
                "FLASH_LOAN_PROVIDER_ADDRESS=",
                vm.toString(flashLoanProvider),
                "\n",
                "TIMELOCK_ADDRESS=",
                vm.toString(deployedContracts.timelock),
                "\n",
                "TIMELOCK=",
                vm.toString(deployedContracts.timelock),
                "\n"
        ...
        vm.writeFile("./.env.simulation", envFile);
        console.log("Simulation env file updated at:", "./.env.simulation");
    }
