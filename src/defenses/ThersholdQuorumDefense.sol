...
/**
 * @title Threshold and Quorum Defense
...
contract ThersholdQuorumDefense is Ownable {
    ...
    function castVote(uint256 proposalId, uint256 forVotes, uint256 againstVotes, uint256 abstainVotes)
        external
        onlyOwner
    {
        ...
        proposalVotes[proposalId] =
            ProposalVotes({forVotes: forVotes, againstVotes: againstVotes, abstainVotes: abstainVotes});
    ...
    function _calculateAverageParticipation() internal view returns (uint256) {
        uint256 len = recentParticipationRates.length;
        if (len == 0) return 0;
        uint256 sum = 0;
