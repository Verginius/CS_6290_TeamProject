...
export function History() {
  ...
  // 辅助函数：获取最后一个数据点的值
  const first = data.frequency[0];
  const last = data.frequency[data.frequency.length - 1];
      ...
      <div className="bg-card rounded-lg p-6 border border-border shadow-lg">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={data.frequency}>
            ...
            />
            <Legend />
            <Line type="monotone" dataKey="flashLoan" stroke="#EF4444" strokeWidth={2} name="Flash Loan" />
            <Line type="monotone" dataKey="whale" stroke="#F59E0B" strokeWidth={2} name="Whale Manipulation" />
            <Line type="monotone" dataKey="spam" stroke="#3B82F6" strokeWidth={2} name="Proposal Spam" />
            <Line type="monotone" dataKey="quorum" stroke="#10B981" strokeWidth={2} name="Quorum Manipulation" />
            <Line type="monotone" dataKey="timelock" stroke="#A855F7" strokeWidth={2} name="Timelock Exploit" />
          </LineChart>
        </ResponsiveContainer>
        <div className="mt-4 grid grid-cols-5 gap-2 text-sm">
          <div className="p-2 bg-destructive/10 rounded-lg border border-destructive/30">
            <div className="text-muted-foreground mb-1">Flash Loan</div>
            <div className="font-bold text-destructive">{first?.flashLoan} → {last?.flashLoan}</div>
          </div>
          <div className="p-2 bg-warning/10 rounded-lg border border-warning/30">
            <div className="text-muted-foreground mb-1">Whale</div>
            <div className="font-bold text-warning">{first?.whale} → {last?.whale}</div>
          </div>
          <div className="p-2 bg-primary/10 rounded-lg border border-primary/30">
            <div className="text-muted-foreground mb-1">Spam</div>
            <div className="font-bold text-primary">{first?.spam} → {last?.spam}</div>
          </div>
          <div className="p-2 bg-success/10 rounded-lg border border-success/30">
            <div className="text-muted-foreground mb-1">Quorum</div>
            <div className="font-bold text-success">{first?.quorum} → {last?.quorum}</div>
          </div>
          <div className="p-2 bg-purple-500/10 rounded-lg border border-purple-500/30">
            <div className="text-muted-foreground mb-1">Timelock</div>
            <div className="font-bold text-purple-500">{first?.timelock} → {last?.timelock}</div>
          </div>
        </div>
      </div>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={data.successRate}>
              <defs>
                ...
                <linearGradient id="spam" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#3B82F6" stopOpacity={0.3} />
                  <stop offset="95%" stopColor="#3B82F6" stopOpacity={0} />
                </linearGradient>
                <linearGradient id="quorum" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor="#10B981" stopOpacity={0.3} />
                  <stop offset="95%" stopColor="#10B981" stopOpacity={0} />
                </linearGradient>
                <linearGradient id="timelock" x1="0" y1="0" x2="0" y2="1">
              ...
              />
              <Area type="monotone" dataKey="flashLoan" stroke="#EF4444" fillOpacity={1} fill="url(#flashLoan)" />
              <Area type="monotone" dataKey="whale" stroke="#F59E0B" fillOpacity={1} fill="url(#whale)" />
              <Area type="monotone" dataKey="spam" stroke="#3B82F6" fillOpacity={1} fill="url(#spam)" />
              <Area type="monotone" dataKey="quorum" stroke="#10B981" fillOpacity={1} fill="url(#quorum)" />
              <Area type="monotone" dataKey="timelock" stroke="#A855F7" fillOpacity={1} fill="url(#timelock)" />
            </AreaChart>
          </ResponsiveContainer>
          <div className="mt-3 space-y-1 text-xs">
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Whale:</span>
              <span className="text-warning">{data.successRate[0]?.whale}% → {data.successRate[data.successRate.length-1]?.whale}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Spam:</span>
              <span className="text-primary">{data.successRate[0]?.spam}% → {data.successRate[data.successRate.length-1]?.spam}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Quorum:</span>
              <span className="text-success">{data.successRate[0]?.quorum}% → {data.successRate[data.successRate.length-1]?.quorum}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Timelock:</span>
              <span className="text-purple-500">{data.successRate[0]?.timelock}% → {data.successRate[data.successRate.length-1]?.timelock}%</span>
            </div>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={data.defenseAdoption}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: '#1E293B', border: '1px solid #334155', borderRadius: '8px' }}
              />
              <Area type="monotone" dataKey="votingDelay" stackId="1" stroke="#10B981" fill="#10B981" fillOpacity={0.6} />
              <Area type="monotone" dataKey="snapshotVoting" stackId="1" stroke="#3B82F6" fill="#3B82F6" fillOpacity={0.6} />
              <Area type="monotone" dataKey="tokenLocking" stackId="1" stroke="#F59E0B" fill="#F59E0B" fillOpacity={0.6} />
              <Area type="monotone" dataKey="dynamicQuorum" stackId="1" stroke="#A855F7" fill="#A855F7" fillOpacity={0.6} />
              <Area type="monotone" dataKey="emergencyPause" stackId="1" stroke="#EF4444" fill="#EF4444" fillOpacity={0.6} />
            </AreaChart>
          </ResponsiveContainer>
          <div className="mt-3 space-y-1 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Voting Delay:</span>
              <span className="text-success">{data.defenseAdoption[0]?.votingDelay}% → {data.defenseAdoption[data.defenseAdoption.length-1]?.votingDelay}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Snapshot Voting:</span>
              <span className="text-primary">{data.defenseAdoption[0]?.snapshotVoting}% → {data.defenseAdoption[data.defenseAdoption.length-1]?.snapshotVoting}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Token Locking:</span>
              <span className="text-warning">{data.defenseAdoption[0]?.tokenLocking}% → {data.defenseAdoption[data.defenseAdoption.length-1]?.tokenLocking}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Dynamic Quorum:</span>
              <span className="text-purple-500">{data.defenseAdoption[0]?.dynamicQuorum}% → {data.defenseAdoption[data.defenseAdoption.length-1]?.dynamicQuorum}%</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-muted-foreground">• Emergency Pause:</span>
              <span className="text-destructive">{data.defenseAdoption[0]?.emergencyPause}% → {data.defenseAdoption[data.defenseAdoption.length-1]?.emergencyPause}%</span>
            </div>
          </div>
        </div>
      </div>
