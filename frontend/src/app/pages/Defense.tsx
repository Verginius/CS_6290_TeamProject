    ...
    <div className="space-y-6">
      ...
      {usingMock && (
        <div className="bg-warning/20 border border-warning rounded-lg p-3 text-sm text-warning">
          ⚠️ Using mock data. Real defense contracts may not be deployed.
        </div>
      )}
      {error && (
      ...
      <div className="bg-card rounded-lg p-4 border border-border shadow-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            >
              <option>Flash Loan Attack</option>
              <option>Whale Manipulation</option>
              <option>Proposal Spam</option>
              <option>Quorum Manipulation</option>
              <option>Timelock Exploit</option>
              {/* <option>Sybil Attack</option>
              <option>Bribery Attack</option>
              <option>Combined Attack</option> */}
            </select>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={saveConfig}
              disabled={loading}
              className="flex items-center gap-2 border border-border hover:bg-secondary px-4 py-2 rounded-lg transition-colors disabled:opacity-50"
            >
              <Save className="w-4 h-4" />
              Save Config
            </button>
      ...
      <div className="bg-card rounded-lg p-6 border border-border shadow-lg">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-secondary/20 rounded-lg border border-border">
            ...
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground w-20">Delay (blocks):</span>
              <input
                type="range"
                min="0"
                max="50000"
                ...
                disabled={!config.votingDelay.enabled}
              />
              <span className="text-sm font-mono w-24">{config.votingDelay.blocks} blocks</span>
            </div>
          </div>
          ...
          <div className="p-4 bg-secondary/20 rounded-lg border border-border">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  checked={config.snapshotVoting.enabled}
                  onChange={(e) => updateNested('snapshotVoting', 'enabled', e.target.checked)}
                  className="w-4 h-4"
                />
                <span className="font-medium">Snapshot Voting</span>
              </div>
              <span className="text-sm text-success">{config.snapshotVoting.enabled ? 'Enabled' : 'Disabled'}</span>
            </div>
            <div className="text-sm text-muted-foreground">
              Vote weight frozen at proposal creation time.
            </div>
          </div>
          ...
          <div className="p-4 bg-secondary/20 rounded-lg border border-border">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  checked={config.tokenLocking.enabled}
                  onChange={(e) => updateNested('tokenLocking', 'enabled', e.target.checked)}
                  className="w-4 h-4"
                />
                <span className="font-medium">Token Locking</span>
              </div>
              <span className="text-sm text-success">{config.tokenLocking.enabled ? 'Enabled' : 'Disabled'}</span>
            </div>
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground w-20">Lock Period (blocks):</span>
              <input
                type="range"
                min="0"
                max="5000"
                step="10"
                value={config.tokenLocking.lockPeriod}
                onChange={(e) => updateNested('tokenLocking', 'lockPeriod', parseInt(e.target.value))}
                className="flex-1"
                disabled={!config.tokenLocking.enabled}
              />
              <span className="text-sm font-mono w-24">{config.tokenLocking.lockPeriod} blocks</span>
            </div>
          </div>
          ...
          <div className="p-4 bg-secondary/20 rounded-lg border border-border">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                <input
                  type="checkbox"
                  checked={config.dynamicQuorum.enabled}
                  onChange={(e) => updateNested('dynamicQuorum', 'enabled', e.target.checked)}
                  className="w-4 h-4"
                />
                <span className="font-medium">Dynamic Quorum</span>
              </div>
              <span className="text-sm text-success">{config.dynamicQuorum.enabled ? 'Enabled' : 'Disabled'}</span>
            </div>
            <div className="grid grid-cols-3 gap-4">
              <div>
                <span className="text-xs text-muted-foreground">Min BPS:</span>
                <input
                  type="number"
                  value={config.dynamicQuorum.minBps}
                  onChange={(e) => updateNested('dynamicQuorum', 'minBps', parseInt(e.target.value))}
                  className="w-full bg-secondary border border-border rounded px-2 py-1 text-sm"
                  disabled={!config.dynamicQuorum.enabled}
                  min={0}
                  max={10000}
                />
              </div>
              <div>
                <span className="text-xs text-muted-foreground">Max BPS:</span>
                <input
                  type="number"
                  value={config.dynamicQuorum.maxBps}
                  onChange={(e) => updateNested('dynamicQuorum', 'maxBps', parseInt(e.target.value))}
                  className="w-full bg-secondary border border-border rounded px-2 py-1 text-sm"
                  disabled={!config.dynamicQuorum.enabled}
                  min={0}
                  max={10000}
              ...
              <div>
                ...
                <input
                  type="number"
                  value={config.dynamicQuorum.windowSize}
                  onChange={(e) => updateNested('dynamicQuorum', 'windowSize', parseInt(e.target.value))}
                  className="w-full bg-secondary border border-border rounded px-2 py-1 text-sm"
                  disabled={!config.dynamicQuorum.enabled}
                  min={1}
                  max={50}
          ...
          <div className="p-4 bg-secondary/20 rounded-lg border border-border">
            ...
            <div className="flex items-center gap-4">
              <span className="text-sm text-muted-foreground w-20">Guardian:</span>
              <input
                type="text"
                value={config.emergencyPause.guardian}
                onChange={(e) => updateNested('emergencyPause', 'guardian', e.target.value)}
                className="flex-1 bg-secondary border border-border rounded px-3 py-1 text-sm font-mono"
                disabled={!config.emergencyPause.enabled}
              />
            </div>
          </div>
